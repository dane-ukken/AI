...
def max_min(board, depth, max_depth):
    ...
    if depth == max_depth:
        ...
        return [helpers.get_static_estimate_for_opening(board), board]
    else:
...
def main():
    ...
    print(helpers.get_output_string_from_board(board))
    swapped_optimum_board = helpers.get_swapped_board(optimum_list[1])
    print(optimum_list[0])
    print(helpers.get_output_string_from_board(swapped_optimum_board))
    print(static_estimator_call_count)
